function bigUpload () {
	...
	this.sendFile = function (chunk) {
		...
		reader.onloadend = function(evt) {
			...
			xhr.open("POST", parent.settings.scriptPath + '/upload/' +
							...
							"&totalchunk=" + parent.uploadData.numberOfChunks, true);
			...
			xhr.onreadystatechange = function() {
				if(xhr.readyState == 4) {
					...
					if(chunk === 0 || parent.uploadData.key === 0) {
						...
						parent.$(parent.settings.resultLink).innerHTML = "<a href=\"" + parent.settings.scriptPath + "/download/" + parent.uploadData.key + "\"> Download Link </a>";
					}
	...
	this.sendFileData = function() {
		...
		xhr = new XMLHttpRequest();
		xhr.open("POST", parent.settings.scriptPath + '/finish/' + this.uploadData.key, true);
		xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
	...
	this.abortFileUpload = function() {
		...
		xhr = new XMLHttpRequest();
		xhr.open("POST", this.settings.scriptPath + '/abort/' + this.uploadData.key, true);
		xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
